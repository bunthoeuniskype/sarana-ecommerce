		...
		<td id="item_table">
			<div id="table_holder" style="width:100%;">
				<table class="report table table-reponsive table-bordered">
					...
					<tbody>
						<?php foreach ($summayData as $key=>$row) { ?>						
						<tr>
						<td>
		             <a class="btn btn-xs btn-info" data-toggle="collapse" href="#detailed{{ $key }}" aria-expanded="false" aria-controls="detailed">Details</a>		           
		            </td>
						...
						<tr class="collapse" id="detailed{{$key}}">
							<td colspan="{{count($headers['summary'])}}">
								<table class="table report table table-reponsive" style="width:100%">
									<thead>
										<tr>
											<?php foreach ($headers['detail'] as $header) { ?>
											<th align="<?php echo $header['align']; ?>"><?php echo $header['data']; ?></th>
											<?php } ?>
										</tr>
									</thead>
									<tbody>
											<?php foreach ($detailData[$key] as $row2) { ?>
